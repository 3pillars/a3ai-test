...
class PricingService {
  ...
  private servicePrices: ServicePrice[] = [
  ...
  private basePriceUSD = 0.05; // $0.05 per OCT
  ...
  async getPriceHistory(period: string, limit: number): Promise<PriceData[]> {
  ...
  async calculateCost(service: string, tier: 'basic' | 'advanced', tokens = 0): Promise<{ oct: string; usd: string }> {
  ...
  async calculateDynamicPrice(basePrice: number, demandFactor: number): Promise<number> {
  ...
  async getBundlePrice(services: string[], discount = 0.1): Promise<{ original: string; discounted: string; savings: string }> {
    let total = 0;
